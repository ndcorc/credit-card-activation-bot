...
import numpy as np
import argparse, imutils, base64, io
import cv2
...
ref = cv2.imread('./data/ocr_reference.png')
...
def get_card_number(img64):
    ...
    cc_type = FIRST_NUMBER[output[0]]
    cc_num = "".join(output)
    print("Credit Card Type: %s" % (cc_type))
    print("Credit Card #: %s" % (cc_num))
